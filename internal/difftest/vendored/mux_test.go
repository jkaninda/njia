...
// Adapted from gorilla/mux v1.8.1 for github.com/jkaninda/njia/muxcompat.
// Test cases and fixtures only; no implementation code was copied.
...
type routeTest struct {
	...
	route           *mux.Route        // the route being tested
	...
	tests := []routeTest{
		{
			...
			route:       new(mux.Route).Host("aaa.bbb.ccc"),
		...
		},
		{
			...
			route:       new(mux.Route).Host("aaa.bbb.ccc:1234"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("aaa.{v1:[a-z]{3}}.ccc:1{v2:(?:23|4)}"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("aaa.{v1:[a-z]{3}}.ccc"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("aaa.{v1:[a-z]{2}(?:b|c)}.ccc"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
			vars:         map[string]string{"v1": "bbb"},
			host:         "aaa.bbb.ccc",
			path:         "",
		...
		},
		{
			...
			route:        new(mux.Route).Host("{v1:[a-z]{3}}.{v2:[a-z]{3}}.{v3:[a-z]{3}}"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("aaa.{v-1:[a-z]{3}}.ccc"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("aaa.{v-1:[a-z]{2}(?:b|c)}.ccc"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
			vars:         map[string]string{"v-1": "bbb"},
			host:         "aaa.bbb.ccc",
			path:         "",
		...
		},
		{
			...
			route:        new(mux.Route).Host("{v-1:[a-z]{3}}.{v-2:[a-z]{3}}.{v-3:[a-z]{3}}"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
	...
	tests := []routeTest{
		{
			...
			route:       new(mux.Route).Path("/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/111/{v1:[0-9]{3}}/333"),
			request:      newRequest("GET", "http://localhost/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/{v1:[0-9]{3}}/{v2:[0-9]{3}}/{v3:[0-9]{3}}"),
			request:      newRequest("GET", "http://localhost/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/{category:a|(?:b/c)}/{product}/{id:[0-9]+}"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/111/{v-1:[0-9]{3}}/333"),
			request:      newRequest("GET", "http://localhost/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/{v-1:[0-9]{3}}/{v-2:[0-9]{3}}/{v-3:[0-9]{3}}"),
			request:      newRequest("GET", "http://localhost/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/{product-category:a|(?:b/c)}/{product-name}/{product-id:[0-9]+}"),
			request:      newRequest("GET", "http://localhost/a/product_name/1"),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/{type:(?i:daily|mini|variety)}-{date:\\d{4,4}-\\d{2,2}-\\d{2,2}}"),
		...
		},
		{
			...
			route:        new(mux.Route).Path(`/{v1:[0-9]*}{v2:[a-z]*}/{v3:[0-9]*}`),
		...
		},
		{
			...
			route:        new(mux.Route).Path("/{category:a|b/c}"),
	...
	tests := []routeTest{
		{
			...
			route:       new(mux.Route).PathPrefix("/111"),
			request:     newRequest("GET", "http://localhost/111/222/333"),
			vars:        map[string]string{},
			host:        "",
			path:        "/111",
		...
		},
		{
			...
			route:       new(mux.Route).PathPrefix("/1"),
			request:     newRequest("GET", "http://localhost/111/222/333"),
			vars:        map[string]string{},
			host:        "",
			path:        "/1",
		...
		},
		{
			...
			route:        new(mux.Route).PathPrefix("/111/{v1:[0-9]{3}}"),
			request:      newRequest("GET", "http://localhost/111/222/333"),
			vars:         map[string]string{"v1": "222"},
			host:         "",
			path:         "/111/222",
		...
		},
		{
			...
			route:        new(mux.Route).PathPrefix("/{v1:[0-9]{3}}/{v2:[0-9]{3}}"),
			request:      newRequest("GET", "http://localhost/111/222/333"),
	...
	tests := []routeTest{
		{
			...
			route:        new(mux.Route).Host("aaa.bbb.ccc").Path("/111/222/333"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
			vars:         map[string]string{},
			scheme:       "http",
			host:         "aaa.bbb.ccc",
		...
		},
		{
			...
			route:        new(mux.Route).Schemes("https").Host("aaa.bbb.ccc").Path("/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("aaa.{v1:[a-z]{3}}.ccc").Path("/111/{v2:[0-9]{3}}/333"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Schemes("ftp", "ssss").Host("aaa.{v1:[a-z]{3}}.ccc").Path("/111/{v2:[0-9]{3}}/333"),
		...
		},
		{
			...
			route:        new(mux.Route).Host("{v1:[a-z]{3}}.{v2:[a-z]{3}}.{v3:[a-z]{3}}").Path("/{v4:[0-9]{3}}/{v5:[0-9]{3}}/{v6:[0-9]{3}}"),
			request:      newRequest("GET", "http://aaa.bbb.ccc/111/222/333"),
	...
	tests := []routeTest{
		{
			...
			route:       new(mux.Route).Headers("foo", "bar", "baz", "ding"),
		...
		},
		{
			...
			route:       new(mux.Route).HeadersRegexp("foo", "ba[zr]"),
	...
	tests := []routeTest{
		{
			...
			route:       new(mux.Route).Methods("GET", "POST"),
	...
	tests := []routeTest{
		{
			...
			route:           new(mux.Route).Queries("foo", "bar", "baz", "ding"),
		...
		},
		{
			...
			route:           new(mux.Route).Host("www.example.com").Path("/api").Queries("foo", "bar", "baz", "ding"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v1}"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v1}", "baz", "{v2}"),
			request:         newRequest("GET", "http://localhost?foo=bar&baz=ding"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v1:[0-9]+}"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v1:[0-9]{1}}"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v1:[0-9]{1}(?:a|b)}"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v-1}"),
			request:         newRequest("GET", "http://localhost?foo=bar"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v-1}", "baz", "{v-2}"),
			request:         newRequest("GET", "http://localhost?foo=bar&baz=ding"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v-1:[0-9]+}"),
			request:         newRequest("GET", "http://localhost?foo=10"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "{v-1:[0-9]{1}(?:a|b)}"),
			request:         newRequest("GET", "http://localhost?foo=1a"),
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", ""),
			request:         newRequest("GET", "http://localhost?foo=bar"),
			vars:            map[string]string{},
			host:            "",
			path:            "",
		...
		},
		{
			...
			route:           new(mux.Route).Queries("foo", "bar"),
...
func TestSchemes(t *testing.T) {
	tests := []routeTest{
		// Schemes
		{
			...
			route:       new(mux.Route).Host("localhost"),
			request:     newRequest("GET", "http://localhost"),
			scheme:      "http",
			host:        "localhost",
			shouldMatch: true,
		},
		{
			...
			route:       new(mux.Route).Schemes("https", "ftp").Host("localhost"),
		...
		},
		{
			...
			route:       new(mux.Route).Schemes("ftp", "https").Host("localhost"),
			request:     newRequest("GET", "ftp://localhost"),
			scheme:      "ftp",
			host:        "localhost",
			shouldMatch: true,
...
func TestMatcherFunc(t *testing.T) {
	m := func(r *http.Request, m *mux.RouteMatch) bool {
	...
	tests := []routeTest{
		{
			...
			route:       new(mux.Route).MatcherFunc(m),
	...
	tests := []routeTest{
		{
			...
			route: new(mux.Route).Path(`/111/{v1:\d}{v2:.*}`).BuildVarsFunc(func(vars map[string]string) map[string]string {
				vars["v1"] = "3"
				vars["v2"] = "a"
				return vars
			}),
		...
		},
		{
			...
			route: new(mux.Route).PathPrefix(`/{v1:\d}`).BuildVarsFunc(func(vars map[string]string) map[string]string {
				vars["v1"] = "2"
				return vars
...
func TestSubRouter(t *testing.T) {
	subrouter1 := new(mux.Route).Host("{v1:[a-z]+}.google.com").Subrouter()
	subrouter2 := new(mux.Route).PathPrefix("/foo/{v1}").Subrouter()
	subrouter3 := new(mux.Route).PathPrefix("/foo").Subrouter()
	subrouter4 := new(mux.Route).PathPrefix("/foo/bar").Subrouter()
	subrouter5 := new(mux.Route).PathPrefix("/{category}").Subrouter()
	tests := []routeTest{
		{
		...
		},
		{
			...
			route:        new(mux.Route).Methods("POST").PathPrefix("/foo").Subrouter().Path("/"),
			request:      newRequest("GET", "http://localhost/foo/"),
			vars:         map[string]string{},
			host:         "",
			path:         "/foo/",
		...
		},
		{
			...
			route:        new(mux.Route).Schemes("https").Subrouter().PathPrefix("/"),
			request:      newRequest("GET", "http://localhost/"),
			vars:         map[string]string{},
			host:         "",
			path:         "/",
		...
		},
		{
			...
			route:        new(mux.Route).Schemes("http").Subrouter().PathPrefix("/"),
			request:      newRequest("GET", "http://localhost/"),
			vars:         map[string]string{},
			host:         "",
			path:         "/",
		...
		},
		{
			...
			route:        new(mux.Route).Headers("X-Forwarded-Proto", "https").Subrouter().PathPrefix("/"),
			request:      newRequest("GET", "http://localhost/"),
			vars:         map[string]string{},
			host:         "",
			path:         "/",
		...
		},
		{
			...
			route:        new(mux.Route).Headers("key", "foobar").Subrouter().PathPrefix("/"),
			request:      newRequest("GET", "http://localhost/"),
			vars:         map[string]string{},
			host:         "",
			path:         "/",
		...
		},
		{
			...
			route:        new(mux.Route).Queries("key", "foobar").Subrouter().PathPrefix("/"),
		...
		},
		{
			...
			route:        new(mux.Route).Schemes("ftp").Host("google.com").Subrouter().Path("/"),
		...
		},
		{
			...
			route:        new(mux.Route).Schemes("https", "ftp").Host("google.com").Subrouter().Schemes("ftp").Path("/"),
			request:      newRequest("GET", "ftp://google.com/"),
			scheme:       "ftp",
			host:         "google.com",
			path:         "/",
...
func TestNamedRoutes(t *testing.T) {
	...
	// gorilla asserted on the unexported r1.namedRoutes map; the equivalent
	// observable behavior is that every name (including names registered on
	// subrouters) resolves through the top-level router's Get.
	for _, name := range []string{"a", "b", "c", "d", "e", "f", "g", "h", "i", "j"} {
		if r1.Get(name) == nil {
			t.Errorf("Expected named route %q to be registered on the parent router", name)
		}
	}
}
...
func TestWalkSingleDepth(t *testing.T) {
	...
	i := 0
	err := r0.Walk(func(route *mux.Route, router *mux.Router, ancestors []*mux.Route) error {
		// gorilla reached into route.matchers[0].(*routeRegexp).template; the
		// exported equivalent is Route.GetPathTemplate.
		tpl, err := route.GetPathTemplate()
		if err != nil {
			t.Fatalf("GetPathTemplate at i = %d: %v", i, err)
		}
		if tpl == "/d" {
		...
		if tpl != "/"+paths[i] {
			t.Errorf(`Expected "/%s" at i = %d; got "%s"`, paths[i], i, tpl)
		}
		i++
		return nil
	})
...
func TestWalkNested(t *testing.T) {
	...
	routeSubrouter := func(r *mux.Route) (*mux.Route, *mux.Router) {
	...
	testCases := []struct {
		...
		{"/g", []*mux.Route{}},
		{"/g/o", []*mux.Route{gRoute}},
		{"/g/o/r", []*mux.Route{gRoute, oRoute}},
		{"/g/o/r/i", []*mux.Route{gRoute, oRoute, rRoute}},
		{"/g/o/r/i/l", []*mux.Route{gRoute, oRoute, rRoute, iRoute}},
		{"/g/o/r/i/l/l", []*mux.Route{gRoute, oRoute, rRoute, iRoute, l1Route}},
		{"/g/o/r/i/l/l/a", []*mux.Route{gRoute, oRoute, rRoute, iRoute, l1Route, l2Route}},
	}

	idx := 0
	err := router.Walk(func(route *mux.Route, router *mux.Router, ancestors []*mux.Route) error {
		path := testCases[idx].path
		// gorilla read route.regexp.path.template directly; GetPathTemplate is
		// the exported equivalent.
		tpl, err := route.GetPathTemplate()
		if err != nil {
			t.Fatalf("GetPathTemplate at idx = %d: %v", idx, err)
		}
		if tpl != path {
...
func TestPanicOnCapturingGroups(t *testing.T) {
	...
	mux.NewRouter().NewRoute().Path("/{type:(promo|special)}/{promoId}.json")
}
...
func getRouteTemplate(route *mux.Route) string {
...
func TestMultipleDefinitionOfSamePathWithDifferentMethods(t *testing.T) {
	...
	t.Run("Get Method with invalid query value should not match", func(t *testing.T) {
		...
		if match.MatchErr != mux.ErrNotFound {
...
func TestSubrouterMatching(t *testing.T) {
	...
	cases := []struct {
		Name                string
		Standard, Subrouter func(*mux.Router)
		Requests            []request
	}{
		{
			"pathPrefix",
			...
			for _, req := range c.Requests {
				t.Run(req.Name, func(t *testing.T) {
					for _, v := range []struct {
						...
						r := mux.NewRouter()
						v.Config(r)
						if r.Match(req.Request, &mux.RouteMatch{}) != v.Expected {
							if v.Expected {
