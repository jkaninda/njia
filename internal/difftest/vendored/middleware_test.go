...
// TestMiddlewareAdd verifies that middlewares registered via Use are actually
// applied. gorilla's original asserted on the unexported router.middlewares
// slice; since that is not reachable through the exported API, registration is
// instead observed through each middleware's invocation count.
func TestMiddlewareAdd(t *testing.T) {
	...
	router.ServeHTTP(NewRecorder(), newRequest("GET", "/"))
	if mw.timesCalled != 1 {
	...
	router.ServeHTTP(NewRecorder(), newRequest("GET", "/"))
	if mw.timesCalled != 2 || mw2.timesCalled != 1 {
	...
	router.Use(banalMw)
	router.ServeHTTP(NewRecorder(), newRequest("GET", "/"))
	if banalCalled != 1 || mw.timesCalled != 3 || mw2.timesCalled != 2 {
